...
} BTNode; // You shouldnot change the definition of BTNode
...

{
    ...
    // leaf까지 내려옴
    if (node->left == NULL && node->right == NULL) // leaf 노드(자식노드 X)
        return leftCount + rightCount;

    if ((node->left == NULL && node->right) || (node->right == NULL && node->left)) // 자식 노드가 하나
