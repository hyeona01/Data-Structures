...

{
    ...
    int leftOddNodes = sumOfOddNodes(node->left);
    int rightOddNodes = sumOfOddNodes(node->right);

    // leaf까지 내려옴
    if (node->item % 2 == 1)
        return leftOddNodes + rightOddNodes + node->item;
    else
        return leftOddNodes + rightOddNodes;
}
